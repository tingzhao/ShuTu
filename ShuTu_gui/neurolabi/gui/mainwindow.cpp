...
void MainWindow::on_actionSWC_Edit_Stat_triggered()
{
  /*
  ZStackFrame *frame = currentStackFrame();
  if (frame != NULL) {
    m_frameInfoDlg->setText(frame->document()->getSwcEditSummary());
    m_frameInfoDlg->
  }
  */
}
...
void MainWindow::on_actionUp_Down_Flip_triggered()
{
  ZStackFrame *frame = activeStackFrame();
  if (frame != NULL) {
    frame->flipStackY();
...
QProgressDialog* MainWindow::getProgressDialog()
{
  if (m_progress == NULL) {
    ...
    m_specialProgressReporter.setProgressBar(m_progress->findChild<QProgressBar*>());
  }
