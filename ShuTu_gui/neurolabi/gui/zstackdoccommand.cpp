...
bool ZStackDocCommand::IsSwcEdit(const QString &msg)
{
  return msg.startsWith("[SWC Edit]");
}

bool ZStackDocCommand::IsSwcEdit(const QUndoCommand *cmd)
{
  if (cmd == NULL) {
    return false;
  }

  return IsSwcEdit(cmd->text());
}

QString ZStackDocCommand::SwcEdit::GetCommandText(const QString &msg)
{
  return "[SWC Edit] " + msg;
}
