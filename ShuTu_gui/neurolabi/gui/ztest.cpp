...
//  stack.load(GET_TEST_DATA_DIR + "/benchmark/block3.tif");
  ...
  std::string dataFile = dataDir + "/DH070613C2X100-20_invert.tif";

  ZStack stack;
  stack.load(dataFile);

  stack.crop(ZIntCuboid(ZIntPoint(232, 787, 47), ZIntPoint(548, 1026, 102)));
  stack.save(dataDir + "/DH070613C2X100-20_invert_cropped.tif");

#endif

#if 0
  std::string dataDir = GET_DATA_DIR + "/paper/neutu/data1/enhanced";
  std::string dataFile = dataDir + "/vaa3d_multiscale.tif";

  ZStack stack;
  stack.load(dataFile);

  stack.crop(ZIntCuboid(ZIntPoint(232, 787, 47), ZIntPoint(548, 1026, 102)));
  stack.save(dataDir + "/vaa3d_multiscale_cropped.tif");
...
//  stack.enhanceLine(3.0, 3.0, 1.0);

//  stack.save(dataDir + "/enhanced/hessian.tif");

  Stack *out = ZStackProcessor::EnhanceLine(stack.c_stack(), 3.0, 3.0, 1.0);
  Stack *out2 = ZStackProcessor::EnhanceLine(stack.c_stack(), 2.0, 2.0, 1.0);
  Stack *out3 = ZStackProcessor::EnhanceLine(stack.c_stack(), 1.0, 1.0, 1.0);

  Stack_Or_E(out, out2, out);
  Stack_Or_E(out, out3, out);

  C_Stack::write(dataDir + "/enhanced/hessian_full.tif", out);
...
#if 0
  std::string dataDir = GET_DATA_DIR + "/system";
  std::string dataFile = dataDir + "/slice15_L11.tif";
...
//  stack.save(dataDir + "/enhanced/hessian.tif");
  ...
  Stack *out = ZStackProcessor::GaussianSmooth(stack.c_stack(), 3.0, 3.0, 1.0);
  C_Stack::write(GET_TEST_DATA_DIR + "/test.tif", out);
  ...
  Stack *out = ZStackProcessor::PartialDiff(stack.c_stack(), 3.0, 3.0, 1.0,
                                            NeuTube::Z_AXIS, NeuTube::Z_AXIS);
