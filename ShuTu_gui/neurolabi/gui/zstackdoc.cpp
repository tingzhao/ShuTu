...
#define infoLockObjectModifiedTypeBufferBufferMutex(locking) \
  if (locking) { \
    LDEBUG() << "+++++++Locking" << &m_objectModifiedTypeBufferMutex; \
  } else { \
    LDEBUG() << "-----UnLocking" << &m_objectModifiedTypeBufferMutex; \
  }
    ...
    */
    ...
    if (hasSwc()) {
      ...
      if (NeutubeConfig::getInstance().getApplication() == "Biocytin") {
        QFileInfo fileInfo(getMainSource().c_str());
        autoSaveDir = fileInfo.absoluteDir().absolutePath().toStdString();
      }
      if (autoSaveDir.empty()) {
        autoSaveDir = NeutubeConfig::getInstance().getPath(
      ...
      if (dir.exists()) {
        ...
        if (NeutubeConfig::getInstance().getApplication() == "Biocytin") {
          if (getStack() != NULL) {
            std::string name;
            if (!getMainSource().empty()) {
              name = ZBiocytinFileNameParser::getCoreName(getMainSource());
            } else {
              name = ZBiocytinFileNameParser::getCoreName(getStack()->sourcePath());
            }
            autoSavePath += "~" + name + ".autosave.swc";
          }
        } else {
  ...
  if (!m_actionMap.contains(item)) {
    ...
    if (action != NULL) {
      ...
      case ZActionFactory::ACTION_SELECT_CONNECTED_SWC_NODE:
        ...
        break;
      case ZActionFactory::ACTION_SELECT_SWC_NODE_INVERSE:
        connect(action, SIGNAL(triggered()), this, SLOT(inverseSwcNodeSelection()));
        break;
...
{
#ifdef _DEBUG_
  infoLockObjectModifiedTypeBufferBufferMutex(true);
#endif
  ...
  for (TStackObjectList::iterator iter = objList.begin(); iter != objList.end();
       ++iter) {
//    role.addRole(m_playerList.removePlayer(*iter));
    bufferObjectModified(*iter, false);
    m_playerList.removePlayer(*iter);
  ...
  notifyObjectModified(false);

#ifdef _DEBUG_
  infoLockObjectModifiedTypeBufferBufferMutex(false);
#endif
}
...
std::set<Swc_Tree_Node*> ZStackDoc::getSelectedSwcNodeSetInRange() const
{
  std::set<Swc_Tree_Node*> swcNodeSet;
  TStackObjectList objList = getObjectList(ZStackObject::TYPE_SWC);
...
QStringList ZStackDoc::toStringList() const
{
  ...
  int currentUndoIndex = undoStack()->index();
  QMap<QString, int> cmdMap;
  for (int i = 0; i < currentUndoIndex; ++i) {
    const QUndoCommand *command = undoStack()->command(i);
    if (ZStackDocCommand::IsSwcEdit(command)) {
      if (cmdMap.contains(command->text())) {
        cmdMap[command->text()]++;
      } else {
        cmdMap[command->text()] = 1;
      }
    }
  }

  for (QMap<QString, int>::const_iterator iter = cmdMap.begin();
       iter != cmdMap.end(); ++iter) {
    int count = iter.value();
    list.append(QString("%1: %2").arg(iter.key()).arg(count));
  }
...
{
  if (getParentDoc()) {
    getParentDoc()->loadFileList(fileList);
    return;
  }
...
void ZStackDoc::executeSwcRescaleCommand(const ZRescaleSwcSetting &setting)
{
  ...
  if (allcommand->childCount() > 0) {
    allcommand->setText(ZStackDocCommand::SwcEdit::GetCommandText("Rescale swc"));
...
bool ZStackDoc::executeSwcNodeExtendCommand(const ZPoint &center)
{
  ...
  if (command != NULL) {
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Extend SWC node"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    deprecateTraceMask();
    succ = true;
  }
...
//            free(branch);
  ...
  if (command != NULL) {
    command->setText(
          ZStackDocCommand::SwcEdit::GetCommandText(
            "Extend SWC node (path computation)"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    deprecateTraceMask();
...
bool ZStackDoc::executeInterpolateSwcZCommand()
{
  ...
  if (hasSelectedSwcNode()) {
    ...
    if (allCommand->childCount() > 0) {
      allCommand->setText(
            ZStackDocCommand::SwcEdit::GetCommandText("Z Interpolation"));
      allCommand->setLogMessage(allCommand->text());
      pushUndoCommand(allCommand);
      deprecateTraceMask();
...
bool ZStackDoc::executeInterpolateSwcPositionCommand()
{
  ...
  if (hasSelectedSwcNode()) {
    ...
    if (allCommand->childCount() > 0) {
      allCommand->setText(ZStackDocCommand::SwcEdit::GetCommandText(
...
bool ZStackDoc::executeInterpolateSwcRadiusCommand()
{
  ...
  if (hasSelectedSwcNode()) {
    ...
    if (allCommand->childCount() > 0) {
      allCommand->setText(ZStackDocCommand::SwcEdit::GetCommandText(
                            QObject::tr("Radius Interpolation")));
      allCommand->setLogMessage(allCommand->text());
      pushUndoCommand(allCommand);
      deprecateTraceMask();
...
bool ZStackDoc::executeSwcNodeChangeZCommand(double z)
{
  ...
  if (hasSelectedSwcNode()) {
    ...
    if (allCommand->childCount() > 0) {
      allCommand->setText(ZStackDocCommand::SwcEdit::GetCommandText(
                            QObject::tr("Change Z of Selected Node")));
      allCommand->setLogMessage(allCommand->text());
      pushUndoCommand(allCommand);
      deprecateTraceMask();
...
bool ZStackDoc::executeTranslateSelectedSwcNode()
{
  ...
  if (!nodeSet.empty()) {
    ...
    if (dlg.exec()) {
      ...
      allCommand->setText(
            ZStackDocCommand::SwcEdit::GetCommandText("Translate SWC node"));
      allCommand->setLogMessage(allCommand->text());
      pushUndoCommand(allCommand);
      deprecateTraceMask();
      return true;
...
//      bool succ = fitter.fitSignal(&newNode, getStack(), channel);
    ...
    if (allCommand->childCount() > 0) {
      allCommand->setText(ZStackDocCommand::SwcEdit::GetCommandText(
                            QObject::tr("Estimate node radius")));
      allCommand->setLogMessage(allCommand->text());
      pushUndoCommand(allCommand);
      deprecateTraceMask();
    } else {
...
bool ZStackDoc::executeMergeSwcNodeCommand()
{
  ...
  if (nodeSet.size() > 1 &&
    ...
    command->setText(
          ZStackDocCommand::SwcEdit::GetCommandText("Merge SWC nodes"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    deprecateTraceMask();
    succ = true;
...
bool ZStackDoc::executeSetRootCommand()
{
  ...
  if (nodeSet.size() == 1) {
    ...
    if (!SwcTreeNode::isRoot(tn)) {
      ...
      command->setText(
            ZStackDocCommand::SwcEdit::GetCommandText("Set SWC root"));
      command->setLogMessage(command->text());
      pushUndoCommand(command);
      succ = true;
  ...
  if (nodeSet.size() == 1) {
    ...
    if (SwcTreeNode::isTurn(tn1, tn, tn2)) {
      ...
      ZUndoCommand *command =
          new ZStackDocCommand::SwcEdit::ChangeSwcNodeGeometry(
            this, tn, x, y, z, r);
      command->setText(ZStackDocCommand::SwcEdit::GetCommandText(
                         "Remove turning node"));
      command->setLogMessage(command->text());
      pushUndoCommand(command);
      deprecateTraceMask();
...
bool ZStackDoc::executeDeleteSwcNodeSetCommand(
    std::set<Swc_Tree_Node *> &nodeSet,
    ZUndoCommand *allCommand, QString &message)
{
  if (!nodeSet.empty()) {
    ...
    if (allCommand->childCount() > 0) {
      ...
      allCommand->setText(ZStackDocCommand::SwcEdit::GetCommandText(
                            QObject::tr("Delete selected node")));
      //      blockSignals(true);
      allCommand->setLogMessage(allCommand->text());
      pushUndoCommand(allCommand);
#ifdef _DEBUG_2
...
bool ZStackDoc::executeDeleteSwcNodeCommandInStackRange()
{
  bool succ = false;
  QString message;

  if (hasSelectedSwcNode()) {
    std::set<Swc_Tree_Node*> nodeSet = getSelectedSwcNodeSetInRange();
    if (!nodeSet.empty()) {
      beginObjectModifiedMode(OBJECT_MODIFIED_CACHE);
      //    QSet<ZStackObject::ETarget> targetSet;
...
//    std::set<Swc_Tree_Node*> nodeSet = getUnselectedSwcNodeSet();
    ...
    if (allCommand->childCount() > 0) {
      message = QString("%1 node(s) are deleted").arg(nodeSet.size());
      allCommand->setText(ZStackDocCommand::SwcEdit::GetCommandText(
                            QObject::tr("Delete unselected node")));
      allCommand->setLogMessage(allCommand->text());
//      blockSignals(true);
      pushUndoCommand(allCommand);
#ifdef _DEBUG_2
...
bool ZStackDoc::executeConnectSwcNodeCommand()
{
  ...
  if (getSelectedSwcNodeNumber() > 1) {
    ...
    command->setText(
          ZStackDocCommand::SwcEdit::GetCommandText("Connect SWC node"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    deprecateTraceMask();
    ...
    Swc_Tree_Node *tn1, Swc_Tree_Node *tn2)
{
  ...
  command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Connect SWC node"));
  command->setLogMessage(command->text());
  pushUndoCommand(command);
  deprecateTraceMask();
...
//            Swc_Tree_Remove_Zigzag(tree.data());
      ...
      command->setText(ZStackDocCommand::SwcEdit::GetCommandText(
                         "Connect SWC node with path computation"));
      command->setLogMessage(command->text());
      pushUndoCommand(command);
      deprecateTraceMask();
...
bool ZStackDoc::executeBreakForestCommand()
{
  if (!m_objectGroup.getSelectedSet(ZStackObject::TYPE_SWC).empty()) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Break SWC forest"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
...
bool ZStackDoc::executeGroupSwcCommand()
{
  if (m_objectGroup.getSelectedSet(ZStackObject::TYPE_SWC).size() > 1) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Group SWC"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
  ...
  if (tree != NULL) {
    ...
    if (hasSwc()) {
      ...
      if (command->childCount() > 0) {
        command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Add SWC branch"));
        command->setLogMessage(command->text());
        pushUndoCommand(command);
        forest->print();
        delete tree;
...
bool ZStackDoc::executeAddSwcCommand(ZSwcTree *tree)
{
  if (tree != NULL) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Add SWC tree"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    deprecateTraceMask();
    return true;
...
bool ZStackDoc::executeReplaceSwcCommand(ZSwcTree *tree)
{
  ...
  if (command->childCount() > 0) {
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Replace SWC tree"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    succ = true;
  } else {
...
bool ZStackDoc::executeAddSwcNodeCommand(const ZPoint &center, double radius)
{
  if (radius > 0) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Add SWC node"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    deprecateTraceMask();
    return true;
    ...
    double x, double y, double z)
{
  ...
  */
  ...
  if (branch.size() > 1) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Trace SWC branch"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
...
bool ZStackDoc::executeInsertSwcNode()
{
  ...
  if (getSelectedSwcNodeNumber() >= 2) {
    ...
    if (command->childCount() > 0) {
      command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Insert SWC node"));
      command->setLogMessage(command->text());
      pushUndoCommand(command);
      deprecateTraceMask();
...
bool ZStackDoc::executeSetBranchPoint()
{
  ...
  if (nodeSet.size() == 1) {
    ...
    if (SwcTreeNode::childNumber(masterRoot) > 1) {
      ...
      command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Set branch point"));
      command->setLogMessage(command->text());
      pushUndoCommand(command);
      deprecateTraceMask();
  ...
  if (nodeSet.size() == 1) {
    ...
    if (SwcTreeNode::childNumber(masterRoot) > 1 || swcList.size() > 1) {
      ...
      if (closestNode != NULL) {
        ...
        command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Connect isolated SWC"));
        command->setLogMessage(command->text());
        pushUndoCommand(command);
        deprecateTraceMask();
    ...
    for (std::vector<Swc_Tree_Node*>::iterator iter = neighborArray.begin();
      ...
      if (SwcTreeNode::isBranchPoint(tn)) {
        ...
        if (hook != NULL) {
          ...
          command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Reset branch point"));
          command->setLogMessage(command->text());
          pushUndoCommand(command);
          deprecateTraceMask();
        }
...
bool ZStackDoc::executeMoveAllSwcCommand(double dx, double dy, double dz)
{
  if (hasSwc()) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Move all SWC"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    ...
    double sx, double sy, double sz, bool aroundCenter)
{
  if (hasSwc()) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Scale all SWC"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    ...
    double sx, double sy, double sz, const ZPoint &center)
{
  ...
  if (!nodeSet.empty()) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Scale SWC node"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    ...
    double theta, double psi, bool aroundCenter)
{
  ...
  if ((!nodeSet.empty() && !aroundCenter) ||
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Rotate SWC node"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
    ...
    double theta, double psi, bool aroundCenter)
{
  if (hasSwc()) {
    ...
    command->setText(ZStackDocCommand::SwcEdit::GetCommandText("Rotate all swc"));
    command->setLogMessage(command->text());
    pushUndoCommand(command);
...
void ZStackDoc::updateSwcPenWidth()
{
  QList<ZSwcTree*> swcList = getSwcList();
  foreach (ZSwcTree *tree, swcList) {
    tree->setDefaultPenWidth(NeutubeConfig::getInstance().getDefaultPenWidth());
  }
}
