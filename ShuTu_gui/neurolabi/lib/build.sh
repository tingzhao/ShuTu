...
then
  ...
  cd fftw-3.3.2
  ./configure --enable-shared=no --disable-fortran --with-pic --prefix=${libdir}/fftw3
  make
  make install

  ./configure --enable-shared=no --disable-fortran --enable-float --with-pic --prefix=${libdir}/fftw3
  make
  make install
  cd ..
fi
