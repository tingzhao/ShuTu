...
static char *WP_ERROR = "White space inside argument angle brackets <>";
...
{ int   has_curly, k;
  ...
  if (Num_of_Units >= Unit_Table_Max)
    { Unit_Table_Max = (int) (1.2*Num_of_Units + 100);
      Unit_Table = (Unit *)
        Guarded_Realloc(Unit_Table,sizeof(Unit)*((size_t) Unit_Table_Max),"Process_Arguments");
    }

  def = Unit_Table + Num_of_Units++;

  def->unit = (Atom **) Guarded_Malloc(sizeof(Atom *)*((size_t) Stop),"Process_Arguments");
...
{ AtStk     = (Atom **) Guarded_Malloc(sizeof(Atom *)*((size_t) mach->longest),"Process_Arguments");
  Stop      = 0;
  Unit_Span = 0;
...
{ int       *has_a_match = (int *)
                             Guarded_Malloc(sizeof(int)*2*((size_t) ArgC),"Process_Arguments");
  int       *chain_cover = has_a_match + ArgC;
  Alpha_Set *all_flags   = (Alpha_Set *)
                             Guarded_Malloc(sizeof(Alpha_Set)*((size_t) ArgC),"Process_Arguments");
...
{ int i, k, v;
  ...
  Last_Req     = 0;
  A_Match      = (Match_State *)
                 Guarded_Malloc(sizeof(Match_State)*((size_t) ArgC),"Process_Arguments");
  F_Path       = (Unit **)
                 Guarded_Malloc(sizeof(Unit *)*((size_t) (ArgC+mach->longest)),"Process_Arguments");
...
{ int    i, j, k, v;
  ...
  { int nvals, atot;   //  Allocate arrays to hold matched values and strings
    ...
    Vtop = vbase = (Value *) Guarded_Malloc(sizeof(Value)*((size_t) nvals),"Process_Arguments");
    Ttop = tbase = (char  *) Guarded_Malloc(((size_t) atot),"Process_Arguments");
    Fill_Values = 1;
  }
...
{ static int   First_Call = 1;
  ...
  { int   lines;
    ...
    if (lines == 0)
      ...
      { Master_Spec = (char **)
                      Guarded_Malloc(sizeof(char *)*((size_t) (lines+1)),"Process_Arguments");
        Master_Spec[lines] = NULL;
        while (--lines >= 0)
          { int   len = (int) (strlen(spec[lines])+2);
            char *pad = Guarded_Malloc(sizeof(char)*((size_t) len),"Process_Arguments");
