...
#define TIFF_EXTRA_SAMPLES          338  //  SHORT  (Samples_Per_Pixel - Photo_Metric_Implied)
...
/* A tiff file consists of a header that "points" to a linked list of image file descriptors
   ...
   If multiple threads all encounter an error at the same time, then only one "grabs" the error
   "resource" and the other error messages get dropped (but the routines still take error exits!).
   To allow subsequent errors to be reported you must "release" this resource by calling
   Tiff_Error_Release before another error message can be captured.
*/
...
Tiff_Reader *Open_Tiff_Reader(const char *name, int *big_endian, int *flag64, int lsm);
...
Tiff_Writer *Open_Tiff_Writer(const char *name, int flag64, int lsm);
...
/* While this reader/writer can read and write LSM's this was done primarily so that we could
   ...
   To help one decide how to map channels in the 2-channel case, Count_LSM_Colors returns the
   number of channels in an LSM, unless the LSM-tag is missing in which case it returns 0.
   Get_LSM_Color returns the colors in the supplied array 'colors' which be large enough
   to hold 'nchannel' colors.  The routine returns 0 if successful, and non-zero value if
   not.
*/

Tiff_IFD *Convert_2_RGB(Tiff_IFD *ifd, int source, int target);

int       Count_LSM_Colors(Tiff_IFD *ifd);
int       Get_LSM_Colors(Tiff_IFD *ifd, int nchannels, int *colors);
